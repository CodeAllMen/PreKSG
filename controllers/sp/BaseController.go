...
func (c *BaseController) getServiceConfig(serviceID string) sp.ServiceInfo {
...
func (c *BaseController) getTrackData() *sp.AffTrack {
	trackID := c.Ctx.Input.Param(":trackID")
	track := new(sp.AffTrack)
	trackIntID, _ := strconv.Atoi(trackID)
...
func (c *BaseController) updateTrack(track *sp.AffTrack) {
	// 更新 Track 表数据
